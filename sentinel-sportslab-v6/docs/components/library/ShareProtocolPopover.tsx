...
import { Copy, CheckCircle2, Share2, Link2, XIcon } from 'lucide-react';

interface ShareProtocolPopoverProps {
    protocolId: string;
    protocolName: string;
    onClose: () => void;
}

export const ShareProtocolPopover: React.FC<ShareProtocolPopoverProps> = ({
    protocolId,
    protocolName,
    onClose,
}) => {
    ...
    const shareUrl = `${window.location.origin}/protocol/${protocolId}`;
    ...
    const handleWhatsApp = () => {
        const message = `Check out this protocol: ${protocolName}\n${shareUrl}`;
        ...
        <div className="fixed inset-0 z-[900] flex items-center justify-center bg-black/30 backdrop-blur-[2px]">
            ...
            >
                ...
                <p className="text-[9px] font-bold text-slate-300 uppercase text-center tracking-wide">
                    Anyone with this link can view the protocol
                </p>
            </div>
        </div>
    );
