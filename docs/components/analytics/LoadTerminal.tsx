...
export const LoadTerminal = ({ loadRegistry, acwrValue, acwrStatus, athleteLoad }) => {
    return (
        <div className="bg-white dark:bg-[#132338] p-10 rounded-xl border border-indigo-100 dark:border-indigo-800/40 shadow-sm space-y-10">
            <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2">
                    <div className="px-4 py-2 bg-indigo-50 dark:bg-indigo-600 rounded-xl text-[10px] font-semibold text-indigo-600 dark:text-white uppercase">Pro Logic V4.2</div>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="p-8 bg-indigo-50/50 rounded-xl border border-indigo-100 dark:border-indigo-800/40 space-y-4">
                    <div className="flex justify-between items-center">
                        <ActivityIcon size={24} className="text-indigo-600 dark:text-indigo-300" />
                ...
                <div className="p-8 bg-white dark:bg-[#1A2D48] rounded-xl border border-slate-100 dark:border-[#243A58] space-y-4 shadow-sm">
                    <div className="flex justify-between items-center">
                        <TrendingUpIcon size={24} className="text-emerald-500" />
                        <span className="text-[8px] font-semibold text-emerald-500 uppercase tracking-wide bg-emerald-50 dark:bg-emerald-900/25 px-2 py-1 rounded-full">+12% vs LW</span>
                    </div>
                    <div>
                        <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Acute Load (7d)</div>
                        <div className="text-4xl font-semibold text-slate-900 dark:text-[#E2E8F0]">{athleteLoad.slice(-7).reduce((acc, r) => acc + (r.sRPE || 0), 0)}</div>
                    </div>
                </div>
                ...
                <div className="p-8 bg-white dark:bg-[#1A2D48] rounded-xl border border-slate-100 dark:border-[#243A58] space-y-4 shadow-sm">
                    ...
                    <div>
                        ...
                        <div className="text-4xl font-semibold text-slate-900 dark:text-[#E2E8F0]">{(athleteLoad.slice(-28).reduce((acc, r) => acc + (r.sRPE || 0), 0) / 4).toFixed(0)}</div>
                    </div>
                </div>
                        ...
                        return (
                            <div key={day} className="flex-1 min-w-[100px] flex flex-col gap-3">
                                ...
                                <div className="flex justify-between items-center px-1">
                                    ...
                                    <span className="text-[10px] font-semibold text-slate-900 dark:text-[#E2E8F0]">{load}</span>
                                </div>
                            </div>
                        );
                    })}
