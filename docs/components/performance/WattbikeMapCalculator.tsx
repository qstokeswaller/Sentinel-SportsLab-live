...
const WattbikeMapCalculator = ({ inline = false }) => {
    ...
    const content = (
        <div className={inline
            ? 'h-full flex flex-col bg-white dark:bg-[#132338]'
            : 'bg-white dark:bg-[#132338] rounded-xl w-full max-w-3xl max-h-[90vh] shadow-xl overflow-hidden flex flex-col border border-slate-200 dark:border-[#243A58]'
        }>

            {/* ─── HEADER ─── */}
            <div className={`${inline ? 'px-6 py-2.5' : 'px-7 py-5'} border-b border-slate-100 dark:border-[#243A58] flex items-center justify-between shrink-0`}>
                <div className="flex items-center gap-3">
                    <div className={`${inline ? 'w-8 h-8' : 'w-10 h-10'} bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-md shrink-0`}>
                        <ClipboardIcon size={inline ? 16 : 18} />
                    </div>
                    <div>
                        <h3 className="text-base font-semibold uppercase tracking-tight text-slate-900 dark:text-[#E2E8F0] leading-none">Map Calculator</h3>
                ...
                <div className="flex items-center gap-3">
                    {/* Tab toggle */}
                    <div className="flex rounded-xl overflow-hidden border border-slate-200 dark:border-[#243A58]">
                        <button
                            onClick={() => setWbMapTab('calc')}
                            className={`${inline ? 'px-4 py-1.5' : 'px-5 py-2'} text-[10px] font-semibold uppercase tracking-wide transition-all ${wbMapTab === 'calc' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-[#132338] text-slate-400 dark:text-[#94A3B8] hover:text-indigo-500'}`}
                        >
                            Calculator
                        </button>
                        <button
                            onClick={() => setWbMapTab('entry')}
                            className={`${inline ? 'px-4 py-1.5' : 'px-5 py-2'} text-[10px] font-semibold uppercase tracking-wide border-l border-slate-200 dark:border-[#243A58] transition-all ${wbMapTab === 'entry' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-[#132338] text-slate-400 dark:text-[#94A3B8] hover:text-indigo-500'}`}
                        >
                            Data Entry
                        </button>
                    </div>
                    {!inline && (
                        <button
                            ...
                            aria-label="Close" className="p-2 hover:bg-slate-100 dark:hover:bg-[#1A2D48] rounded-full text-slate-400 dark:text-[#94A3B8] transition-all"
                        >
                            <XIcon size={18} />
                        </button>
                    )}
            ...
            {/* ─── BODY ─── */}
            <div className={`flex-1 overflow-y-auto ${inline ? 'px-6 py-4' : 'no-scrollbar px-7 py-6'}`}>
                        ...
                        <div className="flex flex-wrap items-end gap-3">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    className="w-24 bg-white dark:bg-[#132338] border border-slate-200 dark:border-[#243A58] rounded-xl px-4 py-2 text-xs font-bold text-slate-700 dark:text-[#CBD5E1] outline-none"
                                />
                            </div>
                            ...
                            {wbMapTargetType !== 'Standalone' && (
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        placeholder="RPM"
                                        className="w-28 bg-white dark:bg-[#132338] border border-slate-200 dark:border-[#243A58] rounded-xl px-4 py-2 text-xs font-bold text-slate-700 dark:text-[#CBD5E1] outline-none placeholder:text-slate-300"
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        {wbMapTargetType === 'Standalone' ? (
                            <div className="flex flex-col items-center py-6 space-y-5">
                                <div className="text-center">
                                    <h4 className="text-lg font-semibold uppercase tracking-tighter text-slate-900 dark:text-[#E2E8F0]">Quick Wattbike Conversion</h4>
                                ...
                                <div className="relative w-full max-w-sm">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 dark:bg-[#0F1C30] border border-slate-200 dark:border-[#243A58] rounded-xl pl-10 pr-16 py-4 text-2xl font-semibold text-slate-900 dark:text-[#E2E8F0] outline-none focus:ring-2 focus:ring-indigo-500/20 text-center"
                                        placeholder="0"
                                    />
                                ...
                                <button
                                    ...
                                    className="w-full max-w-sm py-4 bg-indigo-600 text-white rounded-xl text-[11px] font-semibold uppercase tracking-wide hover:bg-indigo-500 transition-all"
                                >
                            ...
                            <div className="rounded-xl border border-slate-100 dark:border-[#243A58] overflow-hidden">
                                <table className="w-full text-left">
                                    <thead className="bg-slate-50 dark:bg-[#0F1C30] border-b border-slate-100 dark:border-[#243A58]">
                                        <tr>
                                        ...
                                        {calcAthletes.length === 0 ? (
                                            ...
                                            calcAthletes.map(p => {
                                                ...
                                                const fanSetting = baseMap > 0
                                                    ...
                                                    : '–';
                                                return (
                                                    <tr key={p.id} className="border-b border-slate-50 dark:border-[#1A2D48] last:border-0 hover:bg-slate-50/50 dark:hover:bg-[#1A2D48]/60 dark:bg-[#132338]/40 transition-colors">
                                                        <td className="px-5 py-3.5 text-xs font-semibold text-slate-800 dark:text-[#E2E8F0]">{p.name}</td>
                                                        ...
                                                        <td className="px-5 py-3.5 text-xs font-bold text-slate-600 dark:text-[#CBD5E1]">
                                                            {baseMap > 0 ? refRPM : <span className="text-slate-300">—</span>}
                                                        </td>
                                                        <td className="px-5 py-3.5">
                                                            {baseMap > 0 ? (
                                                                <span className="bg-indigo-50 dark:bg-indigo-600 text-indigo-700 dark:text-white px-3 py-1 rounded-lg text-xs font-semibold border border-indigo-100 dark:border-indigo-800/40">
                                                                    {fanSetting}
                                                                </span>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap items-end gap-3">
                                ...
                                <div className="flex rounded-xl overflow-hidden border border-slate-200 dark:border-[#243A58]">
                                    {['Team', 'Individual'].map((t, i) => (
                                        <button
                                            ...
                                            className={`${inline ? 'px-4 py-1.5' : 'px-5 py-2'} text-[10px] font-semibold uppercase tracking-wide transition-all ${i > 0 ? 'border-l border-slate-200 dark:border-[#243A58]' : ''} ${dataEntryType === t ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-[#132338] text-slate-400 dark:text-[#94A3B8] hover:text-indigo-500'}`}
                                        >
                                            {t}
                                        </button>
                                    ))}
                            ...
                            <div className="space-y-1.5">
                                ...
                                <DatePicker value={wbMapDate} onChange={(e) => setWbMapDate(e.target.value)} />
                            </div>
                        </div>
                        ...
                        {entryAthletes.length > 0 ? (
                            <div className="space-y-4">
                                <div className="rounded-xl border border-slate-100 dark:border-[#243A58] overflow-hidden">
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-slate-50">
                                            {entryAthletes.map(p => (
                                                <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-[#1A2D48]/60 dark:bg-[#132338]/40 transition-colors">
                                                    <td className="px-5 py-3 text-xs font-semibold text-slate-800 dark:text-[#E2E8F0]">{p.name}</td>
                                                    <td className="px-5 py-3">
                                                        <input
                                                            ...
                                                            className="w-32 bg-slate-50 dark:bg-[#0F1C30] border border-slate-100 dark:border-[#243A58] rounded-lg px-3 py-1.5 text-xs font-bold outline-none focus:border-indigo-400 focus:bg-white transition-all"
                                                            placeholder="e.g. 300"
                                                        />
                                                    </td>
                                                    <td className="px-5 py-3">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 bg-slate-900 dark:bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-semibold uppercase tracking-wide hover:bg-black dark:hover:bg-indigo-500 transition-all shadow-sm"
                                >
