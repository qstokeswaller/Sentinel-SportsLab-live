...
export const MicrocyclesTab = ({ plan, initialPhaseId = null, initialBlockId = null, initialWeekStart = null, initialSelectedDate = null }) => {
    ...
    // ── Navigation ─────────────────────────────────────────────────────────────
    const goToPrev = () => {
        const idx = currentNavIdx - 1;
        if (idx >= 0) setCurrentWeekStart(allWeeks[idx].date);
    };
    const goToNext = () => {
        const idx = currentNavIdx + 1;
        if (idx < allWeeks.length) setCurrentWeekStart(allWeeks[idx].date);
    };
    ...
    return (
        <div className="space-y-3">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    {currentWeekStart && (
                        <div className="flex items-center gap-2 ml-auto">
                            <button onClick={goToPrev} disabled={currentNavIdx <= 0}
                            ...
                            <button onClick={goToNext} disabled={currentNavIdx >= allWeeks.length - 1}
                                className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-[#1A2D48] disabled:opacity-30 transition-colors">
            ...
            {/* ── Context info strip ───────────────────────────────────────── */}
            {isOutsideBlock && currentWeekStart && (
                contextInfo ? (
                    ...
                    // Fully outside all planned phases — amber warning
