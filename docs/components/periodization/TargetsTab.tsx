...
function PlanGanttStrip({ plan }) {
    ...
    const GANTT_W    = 640;
    const pxLeft     = d => Math.max(0, (daysBetweenGantt(ganttStart, d) / totalDays) * GANTT_W);
    const pxWidth    = (s, e) => Math.max(4, (daysBetweenGantt(s, e || s) / totalDays) * GANTT_W);
    ...
    return (
        <div className="bg-white dark:bg-[#132338] rounded-xl border border-slate-200 dark:border-[#243A58] shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
                <div style={{ width: GANTT_W + 32 + 'px' }} className="px-4 py-3 space-y-1.5">
                    {/* Phase bars */}
                    <div className="relative" style={{ height: '22px' }}>
                        {plan.phases.map(ph => {
                            if (!ph.startDate) return null;
                            const l = pxLeft(ph.startDate);
                            const w = pxWidth(ph.startDate, ph.endDate || ganttEnd);
                            return (
                                <div key={ph.id} title={ph.name}
                                    className="absolute h-full rounded-lg flex items-center px-2 overflow-hidden"
                                    style={{ left: l + 'px', width: w + 'px', backgroundColor: (ph.color || '#6366f1') + '28', border: `1.5px solid ${ph.color || '#6366f1'}80` }}>
                        ...
                        {plan.phases.flatMap(ph => ph.blocks.map(b => ({ ...b, phaseColor: ph.color }))).map(b => {
                            ...
                            return (
                                <div key={b.id} title={b.name}
                                    ...
                                    style={{ left: l + 'px', width: w + 'px', backgroundColor: (b.color || b.phaseColor || '#94a3b8') + '40', border: `1px solid ${b.color || b.phaseColor || '#94a3b8'}60` }}>
                    ...
                    {(plan.events || []).length > 0 && (
                        <div className="relative" style={{ height: '14px' }}>
                            {(plan.events || []).map(ev => {
                                ...
                                const w = pxWidth(ev.date, ev.endDate || ev.date);
                                const color = ev.color || EVENT_TYPE_COLORS[ev.type] || '#6366f1';
                                return (
                                    <div key={ev.id} title={ev.label}
                                        className="absolute h-full rounded flex items-center px-1 overflow-hidden"
                                        style={{ left: l + 'px', width: Math.max(w, 6) + 'px', backgroundColor: color + '30', border: `1.5px solid ${color}` }}>
                    ...
                    {today >= ganttStart && today <= ganttEnd && (
                        <div className="relative" style={{ height: '6px' }}>
                            <div className="absolute top-0 bottom-0 w-0.5 bg-blue-500 rounded-full z-10"
                                style={{ left: pxLeft(today) + 'px' }}>
                                <div className="w-1.5 h-1.5 rounded-full bg-blue-500 -translate-x-[3px] -translate-y-[3px]" />
                            </div>
                        </div>
                    )}
                    {/* Legend */}
                    <div className="flex items-center gap-3 pt-1">
                        ...
                        <div className="flex items-center gap-1.5">
                            <div className="w-4 h-1.5 rounded-sm" style={{ backgroundColor: '#f59e0b30', border: '1.5px solid #f59e0b' }} />
                            <span className="text-[8px] text-slate-400 dark:text-[#64748B]">Event</span>
                        </div>
                    </div>
                </div>
            </div>
