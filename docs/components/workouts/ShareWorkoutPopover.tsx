...
export const ShareWorkoutPopover: React.FC<ShareWorkoutPopoverProps> = ({
    ...
    // NOTE: This popover stays in LIGHT MODE intentionally — it's a preview of the
    // shared link's destination page, which is always rendered light. The recipient
    // (athlete, parent, other coach) views the shared workout on a light page, so
    // the popover matches that experience even when the coach is in dark mode locally.
    return (
        <div className="fixed inset-0 z-[900] flex items-center justify-center bg-black/30 dark:bg-black/60 backdrop-blur-[2px]">
            ...
            >
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-500 truncate max-w-[200px]">{workoutName}</p>
                        </div>
                    </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 flex items-center gap-2">
                    <p className="text-[10px] font-mono text-slate-500 truncate flex-1">{shareUrl}</p>
                    <button
                        onClick={handleCopy}
                ...
                <p className="text-[9px] font-bold text-slate-400 uppercase text-center tracking-wide">
