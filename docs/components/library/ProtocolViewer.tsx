...
const renderTextLines = (lines: TextLine[]) => {
    ...
    while (i < lines.length) {
        ...
        if (line.type === 'heading1') {
            ...
            elements.push(<hr key={line.id} className="border-slate-200 dark:border-[#243A58] my-4" />);
            i++;
                ...
                {exercises.length > 0 ? (
                    <div className="space-y-1.5">
                        {exercises.map((ex, idx) => (
                            <div key={idx} className="flex items-start gap-2.5 bg-white dark:bg-[#132338] rounded-lg border border-slate-200 dark:border-[#243A58] px-3 py-2">
        ...
        return (
            <div className="border border-rose-200 dark:border-rose-900/50 dark:border-rose-800/50 rounded-xl overflow-hidden my-3">
                ...
                <iframe
                    ...
                    className="w-full border-t border-rose-200 dark:border-rose-900/50 h-[75vh] min-h-[600px] bg-white dark:bg-[#132338]"
                />
            </div>
        );
    }
...
export const ProtocolViewer: React.FC<ProtocolViewerProps> = ({ protocol, onBack, onEdit, onDelete, onShare }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-[#132338] px-5 py-4 rounded-xl border border-slate-200 dark:border-[#243A58] shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap shrink-0">
                        ...
                        <button
                            onClick={onDelete}
                            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 dark:hover:bg-red-500/15 transition-all"
                        >
            ...
            {/* Document body */}
            <div className="bg-white dark:bg-[#132338] rounded-xl border border-slate-200 dark:border-[#243A58] shadow-sm px-8 sm:px-14 py-8">
